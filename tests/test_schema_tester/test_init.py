...
def test_calls(caplog):
    ...
    """
    SchemaTester({'type': 'array', 'items': {'type': 'object', 'properties': {}}}, [], None, origin='test')
    assert 'init --> array' == caplog.records[-1].message

    SchemaTester({'type': 'object', 'properties': {}}, {}, None, origin='test')
    assert 'init --> object' == caplog.records[-1].message

    SchemaTester({'type': 'string'}, '', None, origin='test')
