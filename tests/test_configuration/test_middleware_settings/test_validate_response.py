...
def test_non_bool_setting(monkeypatch) -> None:
    ...
    for value in ['', 'test', None, {}, [], 2]:
        with pytest.raises(
            ...
            match='The SWAGGER_TESTER middleware setting `VALIDATE_RESPONSE` must be a boolean value',
        ):
            monkeypatch.setattr(
                django_settings, 'SWAGGER_TESTER', patch_middleware_settings('VALIDATE_RESPONSE', value)
            )
...
def test_bool_setting(monkeypatch) -> None:
    ...
    for value in [True, False]:
        monkeypatch.setattr(django_settings, 'SWAGGER_TESTER', patch_middleware_settings('VALIDATE_RESPONSE', value))
        SwaggerTesterSettings()
