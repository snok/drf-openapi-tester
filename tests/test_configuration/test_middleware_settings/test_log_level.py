...
def test_non_string_log_level(monkeypatch) -> None:
    ...
    for value in [None, {}, [], 2]:
        with pytest.raises(
            ...
            monkeypatch.setattr(django_settings, 'SWAGGER_TESTER', patch_response_middleware_settings('LOG_LEVEL', value))
...
def test_valid_log_level_names(monkeypatch) -> None:
    ...
    for value in ['debug', 'info', 'warning', 'error', 'critical', 'exception']:
        for method in [str.lower, str.upper]:
            monkeypatch.setattr(
                django_settings, 'SWAGGER_TESTER', patch_response_middleware_settings('LOG_LEVEL', method(value))
            )
            SwaggerTesterSettings()
