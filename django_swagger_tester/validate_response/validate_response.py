...
from django_swagger_tester.validate_response.parse.drf_yasg_dynamic_schema import fetch_generated_schema
from django_swagger_tester.validate_response.parse.static_schema import parse_endpoint, load_schema_file
...
def validate_response(response: Response, method: str, endpoint_url: str) -> None:
    ...
    # Fetch schema
    if schema == 'static':
        # If schema is static, we need to pass a path with leading and trailing backslashes
        # /api/v1/cars/correct/

        complete_schema = load_schema_file(path=path)
