    ...
    def _dict(self, response_dict: dict, swagger_schema: Schema) -> None:
        ...
        if len(swagger_schema_keys) != len(response_dict_keys):
            ...
            if len(set(response_dict_keys) - set(swagger_schema_keys)):
                ...
                raise ValidationError(f'The following keys are missing from the swagger schema: {missing_keys}')
            # There are fewer keys returned than documented
            else:
                missing_keys = ', '.join(list(set(swagger_schema_keys) - set(response_dict_keys)))
                raise ValidationError(f'The following keys are missing from the response body schema: {missing_keys}')
...
def openapi_tester(type, path=None):
    def middle(func):
        """
        Wrapper function - enables us to use decoration.
        """

        def swagger_tester(response: Response, swagger_schema: Schema, enforce_camel_case: bool = True) -> None:
            """
            ...
            if not isinstance(response, Response):
                raise ValueError(f'`response` should be a HTTP response object, not a {type(response)}.')

            if not isinstance(swagger_schema, Schema):
        ...
        def wrapper(self, *args, **kwargs):
            # Access method class
            print(f'The path of the schema is "{self.schema_path}"')
            # Add a method
            self.assertMatchingSchema = swagger_tester
            # Run function
